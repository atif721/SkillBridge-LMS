...
const Button: React.FC<ButtonProps> = ({
  ...
  const enableShadow = shadow === undefined ? variant === "primary" || variant === "success" : shadow;
  const enableScale = scaleOnHover === undefined ? size === "xl" : scaleOnHover;

  const baseClasses = clsx(
    `font-medium ${otherProps?.disabled ? "cursor-not-allowed" : "cursor-pointer"}`,
    icon ? "inline-flex items-center gap-2" : "inline-flex items-center justify-center",
    "transition-all duration-300",
  );

  // Determine the actual outline color if variant is 'outline'
  const actualOutlineColor = variant === "outline" ? outlineColor || "indigo" : undefined; // Default to indigo if outline and no color provided
  ...
  const shadowClasses = clsx({
    ...
    "shadow-lg shadow-indigo-300/40 dark:shadow-indigo-500/20": enableShadow && variant === "primary",
    "shadow-lg shadow-emerald-300/40 dark:shadow-emerald-600/20": enableShadow && variant === "success",
