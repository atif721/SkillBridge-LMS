...
export const Sidebar: React.FC<SidebarProps> = ({ userInfo, menus, logoutHandler }) => {
  ...
  return (
    <>
      {/* Mobile backdrop */}
      {isMobileOpen && (
        <div className="fixed inset-0 bg-black/10 z-20 lg:hidden" onClick={() => setIsMobileOpen(false)} />
      )}
      ...
      <Button
        ...
        : <IoMdMenu className="h-6 w-6 text-gray-600 dark:text-gray-400" />}
      </Button>
      <div
        ...
        <div className="flex flex-col h-full">
          <div className="p-4 border-b border-gray-200 dark:border-gray-700">
            <div onClick={() => (window.location.href = "/")} className="cursor-pointer flex items-center space-x-2">
              <div className="h-8 w-8 bg-indigo-600 rounded-md flex items-center justify-center">
                <span className="text-white font-bold">CT</span>
              </div>
              <span className="text-gray-900 dark:text-white font-semibold">CodeTode</span>
            </div>
          </div>
            ...
            {menus?.map((item) => {
              return (
                <div
                  ...
                  className={`${
                    isActive(item?.label) ? "bg-gray-200 text-gray-900 dark:bg-gray-700 dark:text-white" : ""
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <span className="ml-3 text-sm font-medium text-gray-700 dark:text-gray-300">{userInfo.name}</span>
              </div>
              <div className="flex items-center">
                <Button
                  size="sm"
                  ...
                  icon={<MdLogout className="h-4 w-4" />}></Button>
              </div>
            </div>
          </div>
        </div>
